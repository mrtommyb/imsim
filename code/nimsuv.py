...
class ImageSimulation(object):
    ...
    def make_background_stars_image(self, bg_star_magnitudes,
        ...
        jitter_y = np.random.normal(loc=0, scale=self.jitter_pix,
                                    size=self.nstars_in_image)
        x_mean_orig = np.random.uniform(
            high=self.image_shape_pix[0], size=self.nstars_in_image)
        y_mean_orig = np.random.uniform(
            high=self.image_shape_pix[1], size=self.nstars_in_image)
        table_bgstars['x_mean'] = x_mean_orig + jitter_x  # * u.pixel
        table_bgstars['y_mean'] = y_mean_orig + jitter_y  # * u.pixel
        ...
        #  make a unit test to check that with coadds == single integration
        if self.ncoadds > 1:
            for i in range(1, self.ncoadds):
                if noisy:
                    table_bgstars['flux'] = np.random.poisson(fval)
                else:
                    # needed to get the same random state
                    _ = np.random.poisson(fval)
                jitter_x = np.random.normal(loc=0, scale=self.jitter_pix,
                                            size=self.nstars_in_image)
                jitter_y = np.random.normal(loc=0, scale=self.jitter_pix,
                                            size=self.nstars_in_image)
                table_bgstars['x_mean'] = x_mean_orig + jitter_x  # * u.pixel
                table_bgstars['y_mean'] = y_mean_orig + jitter_y  # * u.pixel
                image_bgstars += self._make_gaussian_sources_image(
                    table_bgstars)
    ...
    def make_background_noise_image(self, noise_type='poisson',
                                    noisy=True, rstate=None):
        if rstate is not None:
            np.random.seed(rstate)
        if noisy:
            image_zodi = make_noise_image(
                self.image_shape_pix, type=noise_type,
                mean=self.zodi_ct_per_pix.value *
                self.texp.value * self.ncoadds, random_state=rstate)
